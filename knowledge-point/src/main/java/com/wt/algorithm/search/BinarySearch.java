...
public class BinarySearch {
    ...
    // 二分查找--->递归实现
    public static final <E extends Comparable<E>> int searchR(E[] arr, E target) {
        return searchR(arr, 0, arr.length - 1, target);
    }

    private static final <E extends Comparable<E>> int searchR(E[] arr, int l, int r, E target) {
        if (l > r) return -1;
        int m = l + (r - l) / 2;
        if (arr[m].compareTo(target) > 0) {
            return searchR(arr, l, m - 1, target);
        }
        if (arr[m].compareTo(target) < 0) {
            return searchR(arr, m + 1, r, target);
        }
        return m;
    }

    // 非递归实现二分查找
    public static final <E extends Comparable<E>> int search(E[] arr, E target) {
    ...
    // 非递归实现二分查找--临界点不一样
    public static final <E extends Comparable<E>> int search2(E[] arr, E target) {
    ...
    // >target 的第一个元素的索引
    public static final <E extends Comparable<E>> int upper(E[] arr, E target) {
    ...
    // =target 返回最大的索引
    // >target 的第一个元素的索引
    public static final <E extends Comparable<E>> int ceil(E[] arr, E target) {
    ...
    // >=target 的第一个元素的索引
    public static final <E extends Comparable<E>> int lower_ceil(E[] arr, E target) {
        int l = 0, r = arr.length;
        while (l < r) {
            int mid = l + (r - l) / 2;
            if (arr[mid].compareTo(target) < 0) {
