...
import com.wt.spring.core.convert.ConverterRegistry;
 ...
 * @version 1.0
 * @description
 * @date 2022/9/22 17:24
 */
public class GenericConversionService implements ConversionService, ConverterRegistry {

    private Map<GenericConverter.ConvertiblePair, GenericConverter> converts = new HashMap<>();
    ...
    public void addConverter(Converter<?, ?> converter) {
        ...
        for (GenericConverter.ConvertiblePair convertibleType : converterAdapter.getConvertibleTypes()) {
            converts.put(convertibleType, converterAdapter);
        }
    }
    ...
    public void addConverterFactory(ConverterFactory<?, ?> converterFactory) {
        ...
        for (GenericConverter.ConvertiblePair convertibleType : converterFactoryAdapter.getConvertibleTypes()) {
            converts.put(convertibleType, converterFactoryAdapter);
        }
    }
    ...
    protected GenericConverter getConverter(Class<?> sourceType, Class<?> targetType) {
        ...
        for (Class<?> sourceCandidate : sourceCandidates) {
            for (Class<?> targetCandidate : targetCandidates) {
                ...
                GenericConverter genericConverter = converts.get(convertiblePair);
